...
class BaseTask:
    ...
    def get_discrete_pose(self, env, obj_size, discrete):
        """Get discretized collision-free object pose within specified bounds."""
        max_size = np.sqrt(obj_size[0] ** 2 + obj_size[1] ** 2)
        erode_size = int(np.round(max_size / self.pix_size))

        _, hmap, obj_mask = self.get_true_image(env)

        # Randomly sample an object pose within specified bounds of free-space pixels
        free = np.ones(obj_mask.shape, dtype=np.uint8)
        for obj_ids in env.obj_ids.values():
            for obj_id in obj_ids:
                free[obj_mask == obj_id] = 0
        ...
        # create pixel from selected area
        if discrete == 1:
            pix = [75, 30]
        elif discrete == 2:
            pix = [200, 30]
        ...
        pos = utils.pix_to_xyz(pix, hmap, self.bounds, self.pix_size)
        pos = (pos[0], pos[1], obj_size[2] / 2)
        theta = 0#self.rng.random() * 2 * np.pi
        rot = utils.eulerXYZ_to_quatXYZW((0, 0, theta))
        return pos, rot
